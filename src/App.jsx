...
function App() {
  ...
  const moveTo = (id , x , y) => {
    ...
    if (tile){
      ...
      console.log(id , " x" , dest.x , "y" , dest.y , " " , moveX , " " , moveY);
    ...
    for (let y = 1 ; y < 4 ; y++){
      ...
      if (y2 != y && oldRow[y] != 0){
        ...
        if (oldRow[y] != newRow[y2]){
          console.log(`Pushing : x${to.x}y${to.y}`);
  ...
  function animateRow(newRow, oldRow, dir, x) {
    let workRow = [oldRow[0] , 0 , 0 , 0];
    let y2;
    for (var y = 1 ; y < 4 ; y++){
      y2 = 0;
      while (workRow[y2-1] + oldRow[y] <= newRow[y2-1] || newRow[y2-1] === 0){
        y2--;
      }
      workRow[y2] += oldRow[y];
      if (y2 != y && oldRow[y] != 0){
  ...
  */
  ...
  const popIn = () => {
    ...
    console.log(animFlags.current);
    animFlags.current.forEach(id => {
      const tile = document.getElementById(id);
      if (tile) {
        console.log("animating: " + tile.id);
    ...
    setTimeout(() => {
      animFlags.current.forEach(id => {
        const tile = document.getElementById(id);
        if (tile) {
          console.log("stopping animation for : " + tile.id);
